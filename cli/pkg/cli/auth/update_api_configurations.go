...
// fieldNameToSnakeCase converts camelCase field name to snake_case for proto field mask
func fieldNameToSnakeCase(camelCase string) string {
	var result []rune
...
// buildProviderFieldMask builds a list of snake_case field paths for the field mask.
...
func buildProviderFieldMask(fields ProviderFields, includeAPIKey bool, includeModelID bool, includeModelInfo bool, includeProviderEnums bool) []string {
	...
	if includeProviderEnums {
		fieldPaths = append(fieldPaths, "plan_mode_api_provider", "act_mode_api_provider")
	}

	// Add API key field if requested
	if includeAPIKey {
		fieldPaths = append(fieldPaths, fieldNameToSnakeCase(fields.APIKeyField))
		// Special case: Bedrock also needs secret key
		if fields.APIKeyField == "awsAccessKey" {
			fieldPaths = append(fieldPaths, "aws_secret_key")
		}
	}
	...
	if includeModelID {
		...
		if fields.PlanModeProviderSpecificModelIDField != "" {
			...
			fieldPaths = append(fieldPaths, fieldNameToSnakeCase(fields.PlanModeProviderSpecificModelIDField))
			fieldPaths = append(fieldPaths, fieldNameToSnakeCase(fields.ActModeProviderSpecificModelIDField))
		} else {
			// Provider uses generic fields - update those
			fieldPaths = append(fieldPaths, fieldNameToSnakeCase(fields.PlanModeModelIDField))
			fieldPaths = append(fieldPaths, fieldNameToSnakeCase(fields.ActModeModelIDField))
		}
	}

	// Add model info fields if requested and applicable
	if includeModelInfo && fields.PlanModeModelInfoField != "" {
		fieldPaths = append(fieldPaths, fieldNameToSnakeCase(fields.PlanModeModelInfoField))
		fieldPaths = append(fieldPaths, fieldNameToSnakeCase(fields.ActModeModelInfoField))
	}
...
func RemoveProviderPartial(ctx context.Context, manager *task.Manager, provider cline.ApiProvider) error {
	...
	fieldPaths := []string{fieldNameToSnakeCase(fields.APIKeyField)}
...
func buildBedrockOptionalFieldMask(fields *BedrockOptionalFields) []string {
	...
	if fields.SessionToken != nil {
		fieldPaths = append(fieldPaths, "aws_session_token")
	}
	if fields.Region != nil {
		fieldPaths = append(fieldPaths, "aws_region")
	}
	if fields.UseCrossRegionInference != nil {
		fieldPaths = append(fieldPaths, "aws_use_cross_region_inference")
	}
	if fields.UseGlobalInference != nil {
		fieldPaths = append(fieldPaths, "aws_use_global_inference")
	}
	if fields.UsePromptCache != nil {
		fieldPaths = append(fieldPaths, "aws_bedrock_use_prompt_cache")
	}
	if fields.Authentication != nil {
		fieldPaths = append(fieldPaths, "aws_authentication")
	}
	if fields.UseProfile != nil {
		fieldPaths = append(fieldPaths, "aws_use_profile")
	}
	if fields.Profile != nil {
		fieldPaths = append(fieldPaths, "aws_profile")
	}
	if fields.Endpoint != nil {
		fieldPaths = append(fieldPaths, "aws_bedrock_endpoint")
	}
